...
sam_vitb_checkpoint = "vit_b_medicosam.pt"
sam_model_type = "vit_b"
g_img_size = 256

print(f"Convert SAM to OpenVINO {g_img_size}x{g_img_size} format...")

if not Path(sam_vitb_checkpoint):
    print(f"Model not found: {sam_vitb_checkpoint}")
    quit()

sam = sam_model_registry[sam_model_type](image_size=g_img_size, checkpoint=sam_vitb_checkpoint, num_classes=3)[0]
sam.eval()
...
class SamExportableModel(torch.nn.Module):
    def __init__(
        ...
        print("-----SamExportableModel img_size: ", self.img_size)
    ...
    def forward(
        ...
        if mask_input is None:
            dense_embedding = self.model.prompt_encoder.no_mask_embed.weight.reshape(1, -1, 1, 1).expand(
                point_coords.shape[0], -1, image_embeddings.shape[0], int(g_img_size / 16)
            )
        else:
...
with warnings.catch_warnings():
    ...
    ov_encoder_model = ov.convert_model(
        sam.image_encoder,
        example_input=torch.zeros(1, 3, g_img_size, g_img_size),
        input=([1, 3, g_img_size, g_img_size],),
    )
...
dummy_inputs = {
    ...
    "point_coords": torch.randint(low=0, high=g_img_size, size=(1, 5, 2), dtype=torch.float),
